...
class CodeBertDLC(BrainDLC):
    ...
    def _initialize(self):
        ...
        logging.info(f"CodeBERT 引擎将运行在: {self.device}")

        # 异步加载模型（避免阻塞主线程 UI）
        # 这里为了简单先同步加载，实际生产建议放到线程中
        try:
            # 使用镜像（如果未显式设置）
            logging.info("正在加载 CodeBERT 模型 (microsoft/codebert-base)...")
            self.tokenizer = transformers.AutoTokenizer.from_pretrained("microsoft/codebert-base")
            self.model = transformers.AutoModel.from_pretrained("microsoft/codebert-base")
